...
import { Wrench, Shield, User, RotateCcw, MapPin, ChevronDown } from 'lucide-react';
          ...
          <Link href="/" className="flex items-center gap-2 group shrink-0">
            ...
            <div className="flex flex-col">
              <div className="flex items-center gap-1">
                <span className="text-sm font-black tracking-tight text-[#181528]">WRENCH</span>
          ...
          {isCustomerSection && (
            <div className="relative hidden xs:block">
              ...
              >
                ...
                <span className="truncate max-w-[130px]">{selectedArea}</span>
                <ChevronDown className="w-3 h-3 text-slate-400" />
              </button>

              {showAreaPicker && (
                <div className="absolute top-full left-0 mt-1 bg-white border border-slate-100 rounded-2xl shadow-xl py-2 w-52 z-50 animate-in fade-in zoom-in-95">
          ...
          <button
            onClick={() => {
              confirmModal({
                ...
                message: 'Would you like to reset all demo data with fresh London sample data?',
                type: 'warning',
                confirmText: 'Reset Demo',
                cancelText: 'Keep Current',
                onConfirm: () => {
                  resetDemoData();
                  showSuccess('Demo data reset to London defaults.');
                },
              });
            }}
            className="p-1.5 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-full transition-colors"
            title="Reset London demo data"
          >
